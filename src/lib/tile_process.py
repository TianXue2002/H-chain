...
class Tile():
    ...
    def params(self):
        return [self.w, self.h, self.dx, self.dy]
    
    def copy(self):
        return Tile(self.w, self.h, self.dx, self.dy)

class PlacedTile(Tile):
    def __init__(self, w, h, dx, dy, pos, pauli, t):
        super().__init__(w, h, dx, dy)
        self.pos = pos
        self.pauli = pauli
        self.t = t
    
    def copy(self):
        return PlacedTile(self.w, self.h, self.dx, self.dy, self.pos, self.pauli, self.t)
    
    def params(self):
        return [self.pos, self.w, self.h, self.dx, self.dy]
        ...
        for pair in excitation:
            for i in range(len(pair)):
                if pair[i] < sum(f_orbs):
                    # it's a^dagger
                    for j in range(len(f_orbs)):
                        if pair[i] < sum(f_orbs[0:j+1]):
                            if j == 0:
                                cluster_index = 0
                                cluster_pos = pair[i]
                            else:
                                cluster_index = fragment[j-1]
                                cluster_pos = pair[i] - sum(f_orbs[0:j])
                            break
                    # print(pair[i], cluster_index, cluster_pos, j)
                    pair[i] = cluster_index + cluster_pos
                else:
                    for j in range(len(f_orbs)):
                        if pair[i] < sum(f_orbs[0:j+1]) + sum(f_orbs):
                            if j == 0:
                                cluster_index = 0
                                cluster_pos = pair[i] - sum(f_orbs) + f_orbs[j]
                            else:
                                cluster_index = fragment[j-1]
                                cluster_pos = pair[i] - sum(f_orbs) - sum(f_orbs[0:j]) + f_orbs[j]
                            break
                    # print(pair[i], cluster_index, cluster_pos, j)
                    pair[i] = cluster_index + cluster_pos
...
def process_tiles(tiles, ratio, seam_lst, ifsorted = False):
    ...
    for inter_tiles in inter_tiles_lst:
        for i in range(len(inter_tiles)):
            cur_tile = inter_tiles[i]
            cur_tile.w += int(2 * (ratio - 1))

    post_tiles = []
    intra_tiles_up = sorted(intra_tiles_lst[2], key = lambda tile: tile.dy)
    intra_tiles_mid = sorted(intra_tiles_lst[1], key = lambda tile: ((tile.dy + tile.dx)*tile.dy), reverse=True)
    intra_tiles_down = sorted(intra_tiles_lst[0], key = lambda tile: (tile.dy + tile.dx, tile.dy))

    post_tiles = intra_tiles_up + intra_tiles_mid + intra_tiles_down

    inter_tiles_down = sorted(inter_tiles_lst[0], key = lambda tile: (tile.h + tile.dy, tile.h), reverse=True)
    inter_tiles_up = sorted(inter_tiles_lst[1], key = lambda tile: (tile.h, tile.w))
    ...
    print(sum(tile.w for tile in post_tiles))
    print(sum(tile.w for tile in inter_tiles_up) + sum(tile.w for tile in inter_tiles_down))
    if ifsorted:
        post_tiles = sorted(post_tiles, key=lambda x: sum(x.w * x.h), reverse=True)
    return post_tiles
...
def export_tiles_to_file(tiles, filename):
    with open(filename, "w") as f:
        for tile in tiles:
            ...
            f.write(f"{1}\n")
            f.write(f"{tile.w} {tile.h} {tile.dx} {tile.dy}\n")
...
def read_placed_tiles(filename):
    ...
    try:
        with open(filename, 'r') as file:
            # Read bounding values
            bw_line = file.readline().strip()
            if bw_line.startswith("Bounding Width:"):
                bounding_width = int(bw_line.split(":")[1])
            bh_line = file.readline().strip()
            if bh_line.startswith("Bounding Height:"):
                bounding_height = int(bh_line.split(":")[1])
            ...
            while True:
                ...
                if line.startswith("Placed"):
                    ...
                    _, x, width, height, offsetX, offsetY = line.split()
                    x, width, height, offsetX, offsetY = map(int, [x, width, height, offsetX, offsetY])

                    # Read next two lines for constraint & tag
                    pauli = file.readline().strip()
                    t = file.readline().strip()

                    placed_tiles.append(
                        PlacedTile(width, height, offsetX, offsetY, x, pauli, t)
                    )

        print(f"Bounding Width: {bounding_width}, Height: {bounding_height}")
        print(f"Total tiles: {len(placed_tiles)}")
